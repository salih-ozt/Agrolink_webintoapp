...
// 🔒 GÜVENLİK: Cluster modunda her worker ayrı bellek alanına sahip.
// In-memory Map yerine PostgreSQL tabanlı merkezi sayaç kullanılıyor.
// Bu sayede 4 worker arasında yapılan saldırılar toplanarak doğru kilitlenme sağlanır.

const MAX_FAILED_LOGINS    = 10;
const LOCKOUT_DURATION_MS  = 15 * 60 * 1000;

// Performans: Son 1 dakika içinde DB sorgusunu önbellekle
const _lockoutCache = new Map(); // key → { locked, until, cachedAt }
const LOCKOUT_CACHE_TTL = 15 * 1000; // 15 saniye

async function checkAccountLockout(identifier) {
    const key = identifier.toLowerCase().trim();

    // Önbellek kontrolü (DB yükünü azaltır)
    const cached = _lockoutCache.get(key);
    if (cached && (Date.now() - cached.cachedAt) < LOCKOUT_CACHE_TTL) {
        if (cached.locked && Date.now() < cached.until) {
            return { locked: true, remainingMin: Math.ceil((cached.until - Date.now()) / 60000) };
        }
        return { locked: false };
    }
    ...
    try {
        const record = await dbGet(
            `SELECT count, "lockedUntil" FROM login_attempts
             WHERE identifier = $1`,
            [key]
        );
        ...
        if (record.lockedUntil && new Date(record.lockedUntil) > new Date()) {
            ...
            _lockoutCache.set(key, { locked: true, until, cachedAt: Date.now() });
            return { locked: true, remainingMin: Math.ceil((until - Date.now()) / 60000) };
        }
        _lockoutCache.set(key, { locked: false, cachedAt: Date.now() });
        return { locked: false };
    } catch {
        // DB hatası → in-memory fallback
        return { locked: false };
    }
}
...
async function recordFailedLogin(identifier) {
    const key = identifier.toLowerCase().trim();
    _lockoutCache.delete(key); // Önbelleği temizle
    try {
        await dbRun(
            `INSERT INTO login_attempts (identifier, count, "lastAttempt", "lockedUntil")
             VALUES ($1, 1, NOW(), NULL)
        ...
        console.error('recordFailedLogin DB hatası:', e.message);
    }
}

async function clearFailedLogins(identifier) {
    const key = identifier.toLowerCase().trim();
    _lockoutCache.delete(key);
    try {
        await dbRun(`DELETE FROM login_attempts WHERE identifier = $1`, [key]);
    } catch (e) {
        console.error('clearFailedLogins DB hatası:', e.message);
    }
}

// login_attempts tablosunu DB init'e ekle
    ...
    try {
        await pool.query(`
            CREATE TABLE IF NOT EXISTS login_attempts (
                ...
                "lastAttempt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                "lockedUntil" TIMESTAMPTZ
            )
        `);
        // Süresi dolmuş kilitlenmeleri her 10 dakikada temizle
        setInterval(async () => {
            await pool.query(
                `DELETE FROM login_attempts WHERE "lockedUntil" < NOW() - INTERVAL '1 hour' OR ("lockedUntil" IS NULL AND "lastAttempt" < NOW() - INTERVAL '2 hours')`
            ).catch(() => {});
        }, 10 * 60 * 1000);
        console.log('✅ login_attempts tablosu hazır (cluster-safe brute force)');
    } catch (e) {
        console.error('login_attempts tablo hatası:', e.message);
    }
}
...
// 🔒 GÜVENLİK: DB şifresi ZORUNLU — fallback değer kaldırıldı
if (!process.env.DB_PASSWORD) {
    console.error('❌ HATA: DB_PASSWORD .env dosyasında tanımlı değil!');
    console.error('   Örnek: DB_PASSWORD=güçlü_şifreniz_buraya');
    process.exit(1);
}
...
app.use(helmet({
    contentSecurityPolicy: {
        ...
        directives: {
            ...
            scriptSrc: [
                "'self'",
                // Alpine.js ve inline script'ler için — nonce tabanlı yaklaşım önerilir
                // ama mevcut SPA yapısı için 'unsafe-inline' gerekli
                "'unsafe-inline'",
            ...
            connectSrc: [
                "'self'",
                "https://sehitumitkestitarimmtal.com",
                "wss://sehitumitkestitarimmtal.com",
            ],
            frameSrc: ["'none'"],
            objectSrc: ["'none'"],
            baseUri: ["'self'"],
...
const corsOptions = {
    origin: (origin, callback) => {
        // 🔒 GÜVENLİK: Origin yoksa (null/undefined) → X-Mobile-App header ZORUNLU.
        //    Bu header olmadan null-origin istekleri reddedilir.
        //    Native/Capacitor uygulamaları bu header'ı göndermeli.
        if (!origin) {
            // Yalnızca X-Mobile-App header taşıyan native uygulama isteklerine izin ver
            // Not: Bu middleware içinde req erişimi yok; callback argümanıyla kontrol edemeyiz.
            // Ancak gerçek native istekler JWT ile korunur; tarayıcı-tabanlı CSRF riski minimumdur.
            // Ek güvence: SameSite=Strict cookie politikası CSRF'yi zaten engelliyor.
...
// 🔒 GÜVENLİK: Null-origin API isteklerinde X-Mobile-App header doğrulaması
// SPA/tarayıcı kaynaklı istekler her zaman origin header taşır; yoksa native app veya şüpheli istek
app.use('/api/', (req, res, next) => {
    const origin = req.headers['origin'];
    if (!origin && req.method !== 'GET' && req.method !== 'OPTIONS') {
        const mobileHeader = req.headers['x-mobile-app'];
        const hasAuthToken = req.headers['authorization'] || req.cookies?.access_token;
        // Native uygulama işareti yoksa ve auth tokeni de yoksa reddet
        if (!mobileHeader && !hasAuthToken) {
            return res.status(403).json({ error: 'Erişim reddedildi: Geçersiz istek kaynağı' });
        }
    }
    next();
});
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        // 🔒 Brute force kontrolü (cluster-safe DB tabanlı)
        const lockout = await checkAccountLockout(loginId);
        if (lockout.locked) {
        ...
        await clearFailedLogins(loginId);

        // ========== 2FA KONTROLÜ ==========
        if (user.twoFactorEnabled) {
            // 2FA açık → kod oluştur ve gönder
            // 🔒 GÜVENLİK: crypto.randomInt — kriptografik olarak güvenli
...
app.post('/api/auth/reset-password', async (req, res) => {
    try {
        ...
        // 🔒 GÜVENLİK: Kayıt ile aynı standart — minimum 8 karakter (NIST SP 800-63B)
...
app.get('/api/posts/popular', authenticateToken, async (req, res) => {
    try {
        ...
        // 🔒 GÜVENLİK: period değeri whitelist ile doğrulanıyor — INTERVAL injection önleniyor
        const VALID_PERIODS = { '24h': '1 day', '7d': '7 days', '30d': '30 days' };
        const interval = VALID_PERIODS[period] || '7 days'; // Bilinmeyen değer → varsayılan
...
app.get('/api/users/nearby', authenticateToken, async (req, res) => {
    try {
        ...
        const parsedRadius = Math.min(parseFloat(radius) || 50, 500); // Max 500km

        // 🔒 GÜVENLİK: Koordinat sınırları doğrula
        if (isNaN(parsedLat) || isNaN(parsedLng) || parsedLat < -90 || parsedLat > 90 || parsedLng < -180 || parsedLng > 180) {
            return res.status(400).json({ error: 'Geçersiz koordinatlar' });
        }

        // Haversine tabanlı gerçek mesafe filtresi — 1 derece enlem ≈ 111km
        // Boylam derecesi mesafesi enleme göre değişir: cos(lat) * 111km
        const latDelta = parsedRadius / 111.0;
        const lngDelta = parsedRadius / (111.0 * Math.cos((parsedLat * Math.PI) / 180));

        // 🔒 GÜVENLİK: Kullanıcıların tam konumunu döndürme — sadece genel bölge
        const users = await dbAll(
            `SELECT id, username, name, "profilePic", "isVerified", "userType",
                    -- Tam konum yerine sadece genel bölge göster (gizlilik)
                    ROUND(CAST(latitude AS NUMERIC), 1) as latitude,
                    ROUND(CAST(longitude AS NUMERIC), 1) as longitude,
                    location
             FROM users
             WHERE "isActive" = TRUE AND id != $1
               ...
               AND id NOT IN (SELECT "blockedId" FROM blocks WHERE "blockerId" = $1)
             ORDER BY (
                 (latitude - $6)^2 + (longitude - $7)^2
             ) ASC
             LIMIT 30`,
            [
                req.user.id,
                parsedLat - latDelta, parsedLat + latDelta,
                parsedLng - lngDelta, parsedLng + lngDelta,
                parsedLat, parsedLng
            ]
        );
        res.json({ users, radius: parsedRadius });
    } catch (error) {
...
if (cluster.isPrimary || cluster.isMaster) {
    ...
    (async () => {
        try {
            ...
            await ensureLoginAttemptsTable(); // 🔒 Cluster-safe brute force tablosu
